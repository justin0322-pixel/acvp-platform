...
def _register(client, v, auth_header):
    ...
    return vs_url, int(vs_url.rsplit("/", 1)[1]), session_headers(body)


def _retrieve_prompt(client, sh, vs_url) -> dict:
    for _ in range(50):
...
def _disposition(client, sh, vs_url) -> str:
    for _ in range(50):
        d = client.get(vs_url + "/results", headers=sh).json()[1]["results"]["disposition"]
        if d != "incomplete":
            return d
        time.sleep(0.02)
    return d
...
def test_submit_before_prompt_retrieval_is_conflict(client, acv_version, auth_header):
    vs_url, vs_id, sh = _register(client, acv_version, auth_header)
...
def test_submit_vsid_mismatch_rejected(client, acv_version, auth_header):
    vs_url, vs_id, sh = _register(client, acv_version, auth_header)
    _retrieve_prompt(client, sh, vs_url)
    r = client.post(vs_url + "/results",
...
def test_submit_without_testgroups_rejected(client, acv_version, auth_header):
    ...
    for bad in ({"vsId": vs_id}, {"vsId": vs_id, "testGroups": "nope"},
        ...
        r = client.post(vs_url + "/results",
                        json=[{"acvVersion": acv_version}, bad], headers=sh)
...
def test_submit_unknown_tcid_rejected(client, acv_version, auth_header):
    ...
    r = client.post(vs_url + "/results",
                    json=[{"acvVersion": acv_version}, body], headers=sh)
    assert r.status_code == 400
...
def test_missing_test_cases_yield_missing_disposition(client, acv_version, auth_header):
    ...
    assert _disposition(client, sh, vs_url) == "missing"
...
def test_resubmitting_full_set_clears_missing(client, acv_version, auth_header):
    ...
    client.post(vs_url + "/results",
                json=[{"acvVersion": acv_version}, partial], headers=sh)
    assert _disposition(client, sh, vs_url) == "missing"

    client.put(vs_url + "/results",
               json=[{"acvVersion": acv_version}, golden_response(vs_id)], headers=sh)
    assert _disposition(client, sh, vs_url) == "passed"
